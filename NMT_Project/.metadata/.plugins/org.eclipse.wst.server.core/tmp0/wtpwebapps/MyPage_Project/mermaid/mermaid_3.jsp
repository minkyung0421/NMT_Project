<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>The Little Mermaid</title>
<script
	src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
 <link rel="stylesheet" type="text/css" href="mermaid.css">
   <style>
            table {
           width: 100%;
           border-top: 1px solid #444444;
           border-collapse: collapse;
         }
         th, td {
           border-bottom: 1px solid #444444;
           padding: 10px;
         }
       
a.jcm-top {
    position: fixed;
    right: 15px;
    bottom: 15px;
    border-radius: 5px;
    color: #ffffff;
    text-align: center;
    width: 45px;
    height: 45px;
    font-size: 40px;
    background-color:white;
    z-index: 999;
    display: none;
}
           </style>
</head>
<body>
<a href="#" class="jcm-top hidden-xs hidden-sm"><img src = "../christmas_img/goto.png" width="50px" height="50px"></a>
<script>

$( document ).ready( function() {
$( window ).scroll( function() {
if ( $( this ).scrollTop() > 200 ) {
$( '.jcm-top' ).fadeIn();
} else {
$( '.jcm-top' ).fadeOut();
}
} );
$( '.jcm-top' ).click( function() {
$( 'html, body' ).animate( { scrollTop : 0 }, 400 );
return false;
} );
} );

</script>
 <section class = "section_menu">
                <a href = "../list.jsp"> <button>자유게시판</button></a>
               <article class = "menu">
                <a href = "../index.jsp"><p class = "index">Index</p></a>
                            <ul class = "mylist">
                                <li class = "snow_link"><a href = "../snow/snow_1.jsp" style = "color:black"><label>Snow White</label></a></li>
                                <li class = "mermaid_link"><a href = "../mermaid/mermaid_1.jsp" style = "color:black"><label>The Little Mermaid</label></a></li>
                                <li class = "christmas_link"><a href = "../christmas/christmas_1.jsp" style = "color:black"><label>The Nightmare before Christmas</label></a></li>
                                <li class = "rata_link"><a href = "../rata/rata_1.jsp" style = "color:black"><label>Ratatouille</label></a></li>
                                <li class = "inside_link"><a href = "../inside/inside_1.jsp" style = "color:black"><label>Inside Out</label></a></li>
                                <li class = "moana_link"><a href = "../moana/moana_1.jsp" style = "color:black"><label>Moana</label></a></li>
                                <li class = "koko_link"><a href = "../koko/koko_1.jsp" style = "color:black"><label>Coco</label></a></li>
                            </ul>
                        </article>
        
               
            </section>

            <section class = "section_content">
               <article>
                    <table border="0" width = "90%">
                        <tr>
                            <td style = "text-align: center;">
                                <h1>명대사</h1>
                                <br>
                                "무엇인가 지금 막 시작했다는 것을 알아요" 
                                <br>I know something's starting right now<br><br>
                                "지켜봐줘요 언젠가 보게 될 거예요, 언젠간 그대의 세상에서 살거예요"<br>
                                Watch and you'll see Some day, I'll be Part of your world<br><br>
                            </td>
                        </tr>
                        <tr>
                            <!-- https://blog.naver.com/PostView.nhn?blogId=cine_play&logNo=220958817529&parentCategoryNo=&categoryNo=6&viewDate=&isShowPopularPosts=false&from=postView -->
                            <td style = "padding:3%;">
                                    <label style = "text-align: center"><h1>줄거리</h1></label>  
                                    <img src = "../mermaid_img/mm.jpg" width="40%" height="80%" align="left" style = "margin-right: 2%;">
                                    바다에서 가장 아름다운 목소리의 소유자인 에리얼은 인간에 대한 동경 때문에 아빠인 트라아튼 왕에게 버릇없고 골치아픈 문제아로 찍혀버린다. 
                                    어느날 해일에 말려든 육지의 왕자 에릭을 구하게 되고 그를 바라보며 노래를 부른다. 
                                    왕자가 깨어나자 에리얼은 곧 사라지고 에릭왕자는 자신의 은인은 고운 목소리를 지닌 사람이라는 점만을 기억하게 된다. 
                                    왕자를 구해준 이후로 그를 사랑하게 된 에리얼은 바다의 마녀를 찾아가 자신의 목소리와 인간의 다리를 교환한다.
                                    그리고 3일 안에 왕자의 키스를 얻어내지 못하면 에리얼은 마녀의 소유가 되는 부담까지 안게 된다. 
                                    순순히 모든 조건을 받아 들인 에리얼은 인간이 되어 에릭왕자를 찾지만 에릭은 그녀를 알아보지 못한다.
                            </td>
                        </tr>
                        <td style = "text-align: center; padding:3%">
                            <h1>이스터에그</h1>
                            혹시 이 캐릭터를 아시나요?<br><br>
                            <img src = "../mermaid_img/SEBA.jpg"><br><br>
                            그럼 이 장면은요?<br><br>
                            <img src = "../mermaid_img/eis.jpg" width="50%" height="30%"><br><br>
                            이 캐릭터는 인어공주에 나오는 세바스찬이라는 캐릭터입니다! 영화 '알라딘'에서 지니의 손끝에서
                            마구잡이로 나오던 온갖 잡다한 것들 중에 세바스찬이 껴있었는데요! 알라딘은 세바스찬의 집게에
                            물려 아파하는 모습이 알라딘 영화에서 <br>그대로 나왔습니다.
                           

                        </td>
                    </table>
               </article>
               <a href = "mermaid_4.jsp"><article class = "next_page"><label style="font-weight: 800">OST</label><img src = "../snow_img/nn.png"></article></a>
            </section>
            
            <footer>Copyright&copy;2018 3517 최민경 All rights reserved. </footer>
</body>
</html>